...
export default function AuthPage() {
  ...
  const [email, setEmail] = useState('aleksey@email.com')
  ...
  return (
    <div className="min-h-[calc(100vh-48px)] flex items-center justify-center bg-gradient-to-br from-[#1a1a2e] to-[#0f3460] p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
        ...
        <div className="flex border border-gray-200 rounded-xl overflow-hidden mb-5">
          <button onClick={() => { setMode('login'); setGlobalErr('') }} className={`flex-1 py-2 text-sm font-medium transition-all ${mode === 'login' ? 'bg-[#1a1a2e] text-white' : 'text-gray-500 hover:bg-gray-50'}`}>Войти</button>
          <button onClick={() => { setMode('reg'); setGlobalErr('') }} className={`flex-1 py-2 text-sm font-medium transition-all ${mode === 'reg' ? 'bg-[#1a1a2e] text-white' : 'text-gray-500 hover:bg-gray-50'}`}>Регистрация</button>
        </div>

        {globalErr && <div className="bg-red-50 text-red-800 text-xs rounded-xl p-2.5 mb-3">{globalErr}</div>}
        ...
        {mode === 'login' ? (
          ...
          <div>
            ...
            <div className="mb-3">
              ...
              <input className={`input ${errors.dob ? 'border-red-400' : ''}`} type="date" value={dob} onChange={e => setDob(e.target.value)} />
