...
- URL фотографий машин принимаются только как `https://`; небезопасные схемы вроде `http:` и `javascript:` отбрасываются.
...
- Хранить фото машин в Supabase Storage или доверенном CDN.
...
4. В поле `Фото URL` укажите прямую HTTPS-ссылку на изображение.
5. Сохраните машину.
...
- CSP разрешает HTTPS-изображения, но URL все равно должен быть прямой ссылкой на картинку.
